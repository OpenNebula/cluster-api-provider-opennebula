...
func TestValidateNonSensitiveValues(t *testing.T) {
	tests := []struct {
		...
		{"Envoy Gateway Secret translation options", "config:\n  envoyGateway:\n    extensionManager:\n      hooks:\n        xdsTranslator:\n          translation:\n            secret:\n              includeAll: true\n", false},
		{"case insensitive reference", "EXISTINGSECRET: admin-creds\n", false},
		{"password", "adminPassword: fake-value\n", true},
		{"scalar secret", "secret: fake-value\n", true},
		{"secret container with sensitive value", "secret:\n  password: fake-value\n", true},
		{"secret container with generic value", "secret:\n  value: fake-value\n", true},
		{"secret container with non-boolean includeAll", "secret:\n  includeAll: fake-value\n", true},
