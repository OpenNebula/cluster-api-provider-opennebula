...
func TestReleaseAuthSecretValidationBoundaries(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"wrong namespace", func(app *applicationv1.OneKSApplication) {
			app.Spec.ProtectedSecrets[0].Namespace = "catalogue-workloads"
		}, "InvalidAuthSecretProtectedSecret"},
...
func TestDependencyPlansRejectReleaseAuthSecret(t *testing.T) {
...
func TestProtectedSecretPlanProtectedFieldsAffectDigestWithoutSorting(t *testing.T) {
	base := runAIProtectedPlan(t).Spec
...
func TestProtectedSecretPlanRootMaterializesUnchangedCurrentDependencyChild(t *testing.T) {
	plan := dependencyPlanForTest("dependency", "dependency-chart", nil)
	root := validRootPlanGraph(t, []applicationv1.DependencyReference{dependencyReferenceForPlan(plan)}, []applicationv1.DependencyPlan{plan})
	...
	if child.Spec.PlanVersion != applicationv1.PlanVersion || child.Spec.SecretInputRef != nil || child.Spec.ProtectedSecrets != nil {
...
func TestCurrentPlanWaitsForInputSecretWithoutExecuting(t *testing.T) {
	app := validProtectedRootPlan(t)
	reconciler, recorder := testReconciler(t, app)
	ctx := context.Background()
