...
const (
	...
	ExternalSelectionAnnotation  = "applications.oneks.opennebula.io/external-selection"
	ExternalSelectionExternal    = "External"
	ExternalSelectionManaged     = "Managed"
)
...
type externalLifecycleDecision struct {
	mode           string
	selection      string
...
func (r *Reconciler) resolveExternalLifecycle(ctx context.Context, app *applicationv1.OneKSApplication) (externalLifecycleDecision, error) {
	if !usesExternalDetection(app) {
		return externalLifecycleDecision{}, nil
	}
	mode, err := externalSelection(app)
	if err != nil {
		return externalLifecycleDecision{
			failureReason:  "ExternalSelectionInvalid",
			failureMessage: err.Error(),
		}, nil
	}
	if mode != "" {
		return externalLifecycleDecision{mode: mode}, nil
	}
	detection, err := r.detectExternalDependency(ctx, app)
	if err != nil {
		return externalLifecycleDecision{}, err
	}
	switch detection.state {
	case externalDetectionUsable:
		return externalLifecycleDecision{selection: ExternalSelectionExternal}, nil
	case externalDetectionAbsent:
		return externalLifecycleDecision{selection: ExternalSelectionManaged}, nil
	case externalDetectionUnusable:
		return externalLifecycleDecision{
			failureReason:  "ExternalDependencyUnusable",
			failureMessage: detection.message,
		}, nil
	}
	return externalLifecycleDecision{}, nil
}
