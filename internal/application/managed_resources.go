...
func validateManagedReadiness(readiness applicationv1.ManagedResourceReadiness, path string) *PlanError {
	...
	for index, check := range readiness.Checks {
		if len(validation.IsDNS1123Subdomain(check.Hostname)) != 0 || !validNamespace(check.Service.Namespace) || len(validation.IsDNS1123Subdomain(check.Service.Name)) != 0 {
...
func validGroupVersionKind(apiVersion, kind string) bool {
	if !kubernetesKindPattern.MatchString(kind) {
		return false
	}
...
func reservedOwnershipLabel(key string) bool {
	switch key {
	case LabelApplicationName, LabelApplicationNamespace, LabelApplicationUID, LabelClusterID, LabelManagedBy:
		return true
	default:
		return false
	}
