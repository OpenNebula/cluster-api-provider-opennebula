	...
	for _, resource := range app.Spec.ManagedResources {
		...
		if apierrors.IsNotFound(err) {
			if previous != nil && previous.Phase == "Failed" && previous.Reason == "ReadinessTimeout" {
				status.Phase, status.Reason, status.Message = "Failed", "ReadinessTimeout", previous.Message
		...
		if !ownershipMatches(app, object) {
			status.Phase, status.Reason, status.Message = "Failed", "OwnershipConflict", "Managed object does not have exact OneKS ownership"
			result.markResourceFailed(resource.ID, status.Reason, status.Message)
			result.resources = append(result.resources, status)
			continue
		}
		...
		if ready {
			status.Phase, status.Reason, status.Message = "Ready", "ReadinessSatisfied", "Managed resource is ready"
			result.completed++
		} else {
			...
			if !now.Time.Before(status.ReadinessStartedAt.Add(time.Duration(resource.Readiness.TimeoutSeconds) * time.Second)) {
				status.Phase, status.Reason, status.Message = "Failed", "ReadinessTimeout", truncate(fmt.Sprintf("Managed resource %s exceeded its readiness timeout", resource.ID), 512)
				result.markResourceFailed(resource.ID, status.Reason, status.Message)
			} else {
				status.Phase, status.Reason, status.Message = "Applying", reason, truncate(message, 512)
				result.allResources = false
				if result.current == app.Spec.Release.ReleaseName {
					result.current = resource.ID
				}
...
func deletingManagedResourceStatuses(app *applicationv1.OneKSApplication) []applicationv1.ResourceStatus {
	...
	for _, resource := range app.Spec.ManagedResources {
		statuses = append(statuses, deletingResourceStatus(resource.ID, resource.DeletionPolicy, "Managed resource"))
	}
	return statuses
}
