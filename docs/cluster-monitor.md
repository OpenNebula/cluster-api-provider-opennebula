...
installer Jobs. It does not access Kubernetes Secrets through the Kubernetes
API or read Helm release storage. Kubelet projects the configured authentication
key into the container as a read-only file, which the monitor reads when sending
a report.
